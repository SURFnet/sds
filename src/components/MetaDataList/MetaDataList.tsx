        ...
        <dl className="sds--metadata-list">
            {props.items.map((item, index) =>
                <div key={index} className="sds--metadata-list--item">
                    <dt>{item.label}</dt>
                    {item.values.slice(0, showMoreLess[index] ? item.values.length : cutOffNumber)
                        .map((val,i )=> <dd key={i}>{val}</dd>)}
                    {(!showMoreLess[index] && item.values.length === (cutOffNumber + 1)) &&
                        <dd key={cutOffNumber + 1}>{item.values[cutOffNumber + 1]}</dd>}
                    {item.values.length > (cutOffNumber + 1) &&
